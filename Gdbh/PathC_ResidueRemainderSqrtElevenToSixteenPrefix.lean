...
/-!
# Path C -- relative remainder finite block from sqrt eleven to sixteen

Round 60 closed the `8 ≤ Nat.sqrt n ≤ 10` finite block and left the tail
beginning at `11 ≤ Nat.sqrt n`.  This file closes the next block
`11 ≤ Nat.sqrt n ≤ 16`.  The proof switches from enumerating every divisor
subset to a cardinal bound: throughout the block the residue prime set has at
most five primes, so the double divisor family has at most `32 * 32` pairs.
Each pairwise CRT remainder is bounded by `2n`, giving `2048n`, which is
absorbed by the existing `1 / 11` local-density lower bound.
-/

namespace Gdbh
namespace PathCResidueRemainderSqrtElevenToSixteenPrefix
...
open Gdbh.PathCResidueRemainderSqrtEightToTenPrefix
  (ResidueDoubleDivisorRemainderRelativeAfterSqrtElevenWithConstant
   pathC_kGoldbach_of_remainderAfterSqrtEleven_and_countingInput)
...
private lemma residuePrimeSet_card_le_five_of_eleven_to_sixteen {z : ℕ}
    (hz11 : 11 ≤ z) (hz16 : z ≤ 16) :
    (residuePrimeSet z).card ≤ 5 := by
...
private lemma filteredResiduePowerset_card_le_thirty_two {z k : ℕ}
    (hz11 : 11 ≤ z) (hz16 : z ≤ 16) :
  ...
  have hbase : (residuePrimeSet z).card ≤ 5 :=
    residuePrimeSet_card_le_five_of_eleven_to_sixteen hz11 hz16
  have hpow : 2 ^ (residuePrimeSet z).card ≤ 32 := by
    have hpow' := Nat.pow_le_pow_right (by norm_num : 0 < 2) hbase
    norm_num at hpow'
    exact hpow'
  have hpowerset : (residuePrimeSet z).powerset.card ≤ 32 := by
...
/-- On the block `11 ≤ Nat.sqrt n ≤ 16`, the full signed CRT remainder is
bounded by `2048n`. -/
theorem residueDoubleDivisorRemainderSumAtSqrt_abs_le_two_thousand_forty_eight_mul_n_of_sqrt_eleven_to_sixteen
    {n : ℕ} (hn : 16 ≤ n) (hsqrt_ge_eleven : 11 ≤ Nat.sqrt n)
    (hsqrt_le_sixteen : Nat.sqrt n ≤ 16) :
    |residueDoubleDivisorRemainderSumAtSqrt n| ≤ (2048 * n : ℝ) := by
  let F : Finset (Finset ℕ) :=
    (residuePrimeSet (Nat.sqrt n)).powerset.filter
      (fun d => d.card ≤ canonicalK n)
  have hFcard_nat : F.card ≤ 32 := by
    dsimp [F]
    exact filteredResiduePowerset_card_le_thirty_two
      hsqrt_ge_eleven hsqrt_le_sixteen
  have hFcard_real : (F.card : ℝ) ≤ 32 := by
    exact_mod_cast hFcard_nat
  have hFcard_nonneg : 0 ≤ (F.card : ℝ) := by
    positivity
  have hFcard_sq :
      (F.card : ℝ) * (F.card : ℝ) ≤ 32 * 32 := by
  ...
  change
    |∑ d1 ∈ F, ∑ d2 ∈ F,
        (ArithmeticFunction.moebius (d1.prod id) : ℝ) *
          (ArithmeticFunction.moebius (d2.prod id) : ℝ) *
          residuePairCountingRemainder n d1 d2| ≤ (2048 * n : ℝ)
  calc
    |∑ d1 ∈ F, ∑ d2 ∈ F,
        (ArithmeticFunction.moebius (d1.prod id) : ℝ) *
          (ArithmeticFunction.moebius (d2.prod id) : ℝ) *
          ...
          calc
            (F.card : ℝ) * ((F.card : ℝ) * (2 * n : ℝ))
                = ((F.card : ℝ) * (F.card : ℝ)) * (2 * n : ℝ) := by
                  ring
            _ ≤ (32 * 32 : ℝ) * (2 * n : ℝ) := hmul
            _ = (2048 * n : ℝ) := by ring

/-- Fixed-coefficient finite-block worker for the `11 ≤ Nat.sqrt n ≤ 16`
slice. -/
noncomputable def
    ResidueDoubleDivisorRemainderRelativeSqrtElevenToSixteenAtSqrt
    (A : ℝ) : Prop :=
  0 ≤ A ∧
...
/-- Existential coefficient form of the `11 ≤ sqrt ≤ 16` finite-block worker. -/
noncomputable def
    ResidueDoubleDivisorRemainderRelativeSqrtElevenToSixteenWithConstant :
    Prop :=
  ∃ A : ℝ,
    ResidueDoubleDivisorRemainderRelativeSqrtElevenToSixteenAtSqrt A

/-- Remaining large-range worker after removing the `11 ≤ sqrt ≤ 16` block. -/
noncomputable def
    ResidueDoubleDivisorRemainderRelativeAfterSqrtSeventeenWithConstant :
    Prop :=
  ∃ A : ℝ,
    ResidueDoubleDivisorRemainderRelativeAfterSqrtThreshold 16 A

/-- The `11 ≤ Nat.sqrt n ≤ 16` relative remainder worker is closed with
coefficient `22528`. -/
theorem residueRemainderRelativeSqrtElevenToSixteen_twenty_two_thousand_five_twenty_eight :
    ResidueDoubleDivisorRemainderRelativeSqrtElevenToSixteenAtSqrt
      22528 := by
  refine ⟨by norm_num, ?_⟩
  intro n hn hsqrt_ge_eleven hsqrt_le_sixteen
  have hrem :=
    residueDoubleDivisorRemainderSumAtSqrt_abs_le_two_thousand_forty_eight_mul_n_of_sqrt_eleven_to_sixteen
      hn hsqrt_ge_eleven hsqrt_le_sixteen
  have hlocal_eleventh :
      (1 / 11 : ℝ) ≤ residueDoubleDivisorLocalDensitySumAtSqrt n := by
    rw [
  ...
  have hscale :
      (2048 : ℝ) ≤
        22528 * residueDoubleDivisorLocalDensitySumAtSqrt n := by
    nlinarith
  have hn_nonneg : 0 ≤ (n : ℝ) := by
    exact_mod_cast Nat.zero_le n
  have hmain :
    ...
    calc
      (2048 * n : ℝ)
          ≤
            (22528 * residueDoubleDivisorLocalDensitySumAtSqrt n) *
              (n : ℝ) := hmul
      _ =
          22528 *
            ((n : ℝ) * residueDoubleDivisorLocalDensitySumAtSqrt n) := by
...
/-- Existential form of the closed `11 ≤ sqrt ≤ 16` block. -/
theorem residueRemainderRelativeSqrtElevenToSixteenWithConstant_closed :
    ResidueDoubleDivisorRemainderRelativeSqrtElevenToSixteenWithConstant :=
  ⟨22528,
    residueRemainderRelativeSqrtElevenToSixteen_twenty_two_thousand_five_twenty_eight⟩

/-- A closed `11 ≤ sqrt ≤ 16` block and a `sqrt ≥ 17` tail bound combine into
the Round 60 active `sqrt ≥ 11` target. -/
theorem residueRemainderAfterSqrtElevenFixed_of_sqrtElevenToSixteen_and_afterSqrtSeventeen
    {Ablock Aseventeen : ℝ}
    (hBlock :
      ResidueDoubleDivisorRemainderRelativeSqrtElevenToSixteenAtSqrt Ablock)
    (hSeventeen :
      ResidueDoubleDivisorRemainderRelativeAfterSqrtThreshold 16 Aseventeen) :
    ResidueDoubleDivisorRemainderRelativeAfterSqrtThreshold 10
      (max Ablock Aseventeen) := by
  rcases hBlock with ⟨hAblock, hBlockBd⟩
  rcases hSeventeen with ⟨_hAseventeen, hSeventeenBd⟩
  refine ⟨hAblock.trans (le_max_left Ablock Aseventeen), ?_⟩
  ...
  · have hbd := hBlockBd n hn hsqrt_ge_eleven hsqrt_le_sixteen
    have hscale :
        Ablock * ((n : ℝ) * residueDoubleDivisorLocalDensitySumAtSqrt n)
          ≤ max Ablock Aseventeen *
              ((n : ℝ) * residueDoubleDivisorLocalDensitySumAtSqrt n) :=
      mul_le_mul_of_nonneg_right (le_max_left Ablock Aseventeen) hmain_nonneg
    exact hbd.trans hscale
  · have hsqrt_ge_seventeen : 16 + 1 ≤ Nat.sqrt n := by
      exact Nat.succ_le_iff.mpr (lt_of_not_ge hsqrt_le_sixteen)
    have hbd := hSeventeenBd n hn hsqrt_ge_seventeen
    have hscale :
        Aseventeen * ((n : ℝ) * residueDoubleDivisorLocalDensitySumAtSqrt n)
          ≤ max Ablock Aseventeen *
              ((n : ℝ) * residueDoubleDivisorLocalDensitySumAtSqrt n) :=
      mul_le_mul_of_nonneg_right (le_max_right Ablock Aseventeen)
        hmain_nonneg
    exact hbd.trans hscale

/-- Existential bridge from the two smaller workers to the Round 60 active
`sqrt ≥ 11` target. -/
theorem residueRemainderAfterSqrtEleven_of_sqrtElevenToSixteen_and_afterSqrtSeventeen
    (hBlock :
      ResidueDoubleDivisorRemainderRelativeSqrtElevenToSixteenWithConstant)
    (hSeventeen :
      ResidueDoubleDivisorRemainderRelativeAfterSqrtSeventeenWithConstant) :
    ResidueDoubleDivisorRemainderRelativeAfterSqrtElevenWithConstant := by
  rcases hBlock with ⟨Ablock, hAblock⟩
  rcases hSeventeen with ⟨Aseventeen, hAseventeen⟩
  exact ⟨max Ablock Aseventeen,
    residueRemainderAfterSqrtElevenFixed_of_sqrtElevenToSixteen_and_afterSqrtSeventeen
      hAblock hAseventeen⟩

/-- Closing the `11 ≤ sqrt ≤ 16` block means only the `sqrt ≥ 17` tail remains. -/
theorem residueRemainderAfterSqrtEleven_of_afterSqrtSeventeen
    (hSeventeen :
      ResidueDoubleDivisorRemainderRelativeAfterSqrtSeventeenWithConstant) :
    ResidueDoubleDivisorRemainderRelativeAfterSqrtElevenWithConstant :=
  residueRemainderAfterSqrtEleven_of_sqrtElevenToSixteen_and_afterSqrtSeventeen
    residueRemainderRelativeSqrtElevenToSixteenWithConstant_closed hSeventeen

/-- Final Path C adapter after closing the `11 ≤ sqrt ≤ 16` block. -/
theorem pathC_kGoldbach_of_remainderAfterSqrtSeventeen_and_countingInput
    (hSeventeen :
      ResidueDoubleDivisorRemainderRelativeAfterSqrtSeventeenWithConstant)
  ...
  pathC_kGoldbach_of_remainderAfterSqrtEleven_and_countingInput
    (residueRemainderAfterSqrtEleven_of_afterSqrtSeventeen hSeventeen)
    hCounting

end PathCResidueRemainderSqrtElevenToSixteenPrefix
end Gdbh

#print axioms
  Gdbh.PathCResidueRemainderSqrtElevenToSixteenPrefix.residueDoubleDivisorRemainderSumAtSqrt_abs_le_two_thousand_forty_eight_mul_n_of_sqrt_eleven_to_sixteen
#print axioms
  Gdbh.PathCResidueRemainderSqrtElevenToSixteenPrefix.residueRemainderRelativeSqrtElevenToSixteen_twenty_two_thousand_five_twenty_eight
#print axioms
  Gdbh.PathCResidueRemainderSqrtElevenToSixteenPrefix.pathC_kGoldbach_of_remainderAfterSqrtSeventeen_and_countingInput
